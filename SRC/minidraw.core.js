...
class LineObject extends BaseFigure {
    ...
    mouse_down(x, y) {
        let { x_p, y_p } = this.model.coordinatesPreprocess({ x, y });
        this.prevX = x_p;
        this.prevY = y_p;
...
class Model {
    ...
    coordinatesPreprocess(coordinates) {
        let { x, y } = coordinates;
        if (this.snapTo10)
        {
    ...
    mouse_move(e) {
        if (this.#Status != Statuses.None) {
            let { x_p, y_p } = this.coordinatesPreprocess(this.getMousePos(this.svg, e));
            
            if (this.#Status == Statuses.CustomMove) {
                this.#SelectedObject.mouse_move(x_p, y_p);
            }

            if (this.#Status == Statuses.MoveMeAutomaticaly) {
                let dx = x_p - this.prevX;
                let dy = y_p - this.prevY;
            ...
            if (this.#Status == Statuses.DrawConnector) {
                this.#SelectedObject.drawTo(x_p, y_p);
            }
            this.update_svg_size();
        }
    }
    mouse_down(e) {
        this.#SelectedObject = null;
        let { x, y } = this.getMousePos(this.svg, e);
        let { x_p, y_p } = this.coordinatesPreprocess({ x, y });
